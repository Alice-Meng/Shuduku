...
import com.mdh.shuduku.game.ShudukuGame;
 ...
 *
 */
public class ShudukuDatabase {
	public static final String DATABASE_NAME = "OpenShuduku";
    
    
    public static final String SUDOKU_TABLE_NAME = "shuduku";
    ...
    public ShudukuDatabase(Context context) {
    ...
    public FolderInfo getFolderInfoFull(long folderID) {
    	...
    	try
        {
	    	...
	    	String q = "select " + FOLDER_TABLE_NAME+"."+FolderColumns._ID + " as _id, "
	    			+ FOLDER_TABLE_NAME+"."+FolderColumns.NAME + " as name, "
	    			+ SUDOKU_TABLE_NAME + "." + ShudukuColumns.STATE + " as state, "
	    			+ "count("+SUDOKU_TABLE_NAME + "." + ShudukuColumns.STATE + ") as count "
	    			+ "from " + FOLDER_TABLE_NAME + " left join " + SUDOKU_TABLE_NAME
	    			+ " on " + FOLDER_TABLE_NAME+"."+FolderColumns._ID + " = " + SUDOKU_TABLE_NAME + "." + ShudukuColumns.FOLDER_ID
	    			+ " where " + FOLDER_TABLE_NAME+"."+FolderColumns._ID + " = " + folderID
	    			+ " group by " + SUDOKU_TABLE_NAME + "." + ShudukuColumns.STATE;
	        ...
	        while (c.moveToNext()) {
	        	...
	        	if (state == ShudukuGame.GAME_STATE_COMPLETED) {
	        		folder.solvedCount += count;
	        	}
	        	if (state == ShudukuGame.GAME_STATE_PLAYING) {
    ...
    public void deleteFolder(long folderID) {
    	...
    	db.delete(SUDOKU_TABLE_NAME, ShudukuColumns.FOLDER_ID + "=" + folderID, null);
    	// delete the folder
...
//        qb.appendWhere(ShudukuColumns.FOLDER_ID + "=" + folderID);
//        
//        if (filter != null) {
//        	if (!filter.showStateCompleted) {
//        		qb.appendWhere(" and " + ShudukuColumns.STATE + "!=" + ShudukuGame.GAME_STATE_COMPLETED);
//        	}
//        	if (!filter.showStateNotStarted) {
//        		qb.appendWhere(" and " + ShudukuColumns.STATE + "!=" + ShudukuGame.GAME_STATE_NOT_STARTED);
//        	}
//        	if (!filter.showStatePlaying) {
//        		qb.appendWhere(" and " + ShudukuColumns.STATE + "!=" + ShudukuGame.GAME_STATE_PLAYING);
//        	}
//        }
//        
//        SQLiteDatabase db = mOpenHelper.getReadableDatabase();
...
//    }
     ...
     * @return
     */
    public ShudukuGame getSudoku(long sudokuID) {
        SQLiteQueryBuilder qb = new SQLiteQueryBuilder();

        qb.setTables(SUDOKU_TABLE_NAME);
        qb.appendWhere(ShudukuColumns._ID + "=" + sudokuID);
        ...
        try {
        	...
        	if (c.moveToFirst()) {
            	long id = c.getLong(c.getColumnIndex(ShudukuColumns._ID));
            	long created = c.getLong(c.getColumnIndex(ShudukuColumns.CREATED));
            	String data = c.getString(c.getColumnIndex(ShudukuColumns.DATA));
            	long lastPlayed = c.getLong(c.getColumnIndex(ShudukuColumns.LAST_PLAYED));
            	int state = c.getInt(c.getColumnIndex(ShudukuColumns.STATE));
            	long time = c.getLong(c.getColumnIndex(ShudukuColumns.TIME));
            	String note = c.getString(c.getColumnIndex(ShudukuColumns.PUZZLE_NOTE));
            	
            	s = new ShudukuGame();
            	s.setId(id);
            	s.setCreated(created);
            	s.setCells(data);//CellCollection.deserialize(data));
            	s.setLastPlayed(lastPlayed);
            	s.setState(state);
            	s.setTime(time);
            	s.setPuzzleNoteString(note);
        	}
        } finally {
        	if (c != null) c.close();
        }
     ...
     * @return
     */
    public long insertSudoku(long folderID, ShudukuGame sudoku) {
    	SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        ContentValues values = new ContentValues();
//        values.put(ShudukuColumns.DATA, sudoku.getCells().serialize());
        values.put(ShudukuColumns.CREATED, sudoku.getCreated());
        values.put(ShudukuColumns.LAST_PLAYED, sudoku.getLastPlayed());
        values.put(ShudukuColumns.STATE, sudoku.getState());
        values.put(ShudukuColumns.TIME, sudoku.getTime());
        values.put(ShudukuColumns.PUZZLE_NOTE, sudoku.getPuzzleNoteString());
        values.put(ShudukuColumns.FOLDER_ID, folderID);
...
//	}
     ...
     * @param sudoku 
     */
    public void updateSudoku(ShudukuGame sudoku) {
        ContentValues values = new ContentValues();
//        values.put(ShudukuColumns.DATA, sudoku.getCells().serialize());
        values.put(ShudukuColumns.LAST_PLAYED, sudoku.getLastPlayed());
        values.put(ShudukuColumns.STATE, sudoku.getState());
    	...
    	SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        db.update(SUDOKU_TABLE_NAME, values, ShudukuColumns._ID + "=" + sudoku.getId(), null);
    }
    ...
    public void deleteSudoku(long sudokuID) {
		SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        db.delete(SUDOKU_TABLE_NAME, ShudukuColumns._ID + "=" + sudokuID, null);
    }
